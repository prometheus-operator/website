...
for subdir in implemented accepted; do
  for f in "repos/prometheus-operator/Documentation/proposals/$subdir/"*.md; do
    dest=$(basename "$f" | sed 's/^[0-9]*-//')
    cp "$f" "content/docs/proposals/$subdir/$dest"
  done
done
...
cp repos/kube-prometheus/docs/customizations/perses.md content/kube-prometheus/kube/perses.md
